...
> **HARD RULE - no em dashes.** Never use an em dash or an en dash (the long dash characters) anywhere in the article body, title, or excerpt. They are the single clearest "AI tell" and the user has banned them outright. Use a comma, a period, parentheses, or a plain hyphen instead; number ranges use a hyphen ("10-20 euros"). Non-negotiable: the published site must contain zero long dashes. A deterministic build step (scripts.scrub_dashes) strips any that slip through, but write clean so it never has to.
