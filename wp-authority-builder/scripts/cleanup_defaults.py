...
def main() -> int:
    ...
    for slug, ptype in (("hello-world", "post"), ("sample-page", "page"), ("privacy-policy", "page")):
        ids = c.wp(["post", "list", f"--post_type={ptype}", f"--name={slug}", "--post_status=any", "--field=ID"]).strip()
        for pid in ids.split():
            # keep an auto-created Privacy Policy draft only if it's a draft we will replace later; here remove the stub
            try:
