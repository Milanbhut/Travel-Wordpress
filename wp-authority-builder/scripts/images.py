...
def import_image_to_wp(c, cfg, url, filename, post_id=None, featured=False, title=None, alt=None, max_w=1600):
    """Download+compress an image, SFTP it to the server, then `wp media import` it.
    ...
    remote_tmp = posixpath.join("/tmp", filename)
    ...
    args = ["media", "import", remote_tmp, "--porcelain"]
    if post_id:
    ...
    try:
        ...
        c._ssh_exec("rm -f " + shlex.quote(remote_tmp))
